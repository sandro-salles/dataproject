...
class CheckoutCriteria(models.Model):
    ...
    class Meta:
        verbose_name = _('Criterio de seleção de dados')
        verbose_name_plural = _('Criterios de seleção de dados')
...
class Cart(DatableModel):
    ...
    status = models.CharField(_('Status'), max_length=14, db_index=True, choices=Cart.STATUS_CHOICES, default=Cart.STATUS_CHOICES_CREATED[0])
...
class Purchase(PolymorphicModel, DatableModel):
    ...
    price_per_unit = models.FloatField(_('Preço por registro'))
    price_per_unit_range = models.IntegerRangeField(_('Faixa de preço por registro'))

    method = models.CharField(_(u'Método de pagamento'), max_length=14, db_index=True, choices=PAYMENT_METHOD_CHOICES, default=PAYMENT_METHOD_CHOICES_BILLET[0])
    num_installments = models.IntegerField(_('Número de parcelas'), default=1)
...
class Payment(models.Model):
    ...
    STATUS_CHOICES = (STATUS_CHOICES_AWAITING, STATUS_CHOICES_LATE, STATUS_CHOICES_SETTLED)

    purchase = models.ForeignKey(Purchase)
    status = models.CharField(_('Status'), max_length=14, db_index=True, choices=Payment.STATUS_CHOICES, default=Payment.STATUS_CHOICES_AWAITING[0])
    total = models.FloatField(_('Valor total do pagamento'))

    ref_installment = models.IntegerField(_(u'Parcela de referência'), default=1)
...
class Collection(models.Model):
    purchase = models.ForeignKey(Purchase)
    items = models.ManyToManyField(Person, through='CollectionItem')
